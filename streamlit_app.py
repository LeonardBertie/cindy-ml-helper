...
# ========== 展示最新一问一答（在主页面展示） ==========
  ...
  # 页面3：模型训练
    ...
    if st.button("请给我解释一下以上代码"):
      st.write("Y_train = label[X_train_ind]  # 根据训练集索引提取对应的标签")
      st.write("Y_test = label[X_test_ind]    # 根据测试集索引提取对应的标签")
      st.write("X_train_ind 和 X_test_ind 是之前划分好的训练集和测试集样本索引")
      st.write("通过索引从完整的 label 数组中提取对应位置的标签")
      st.write("最终得到与特征数据对应的训练标签 Y_train 和测试标签 Y_test")
  ...
  # 页面4：模型训练
    ...
    if st.button("这几句代码是什么意思"):
      st.write("import pandas as pd  # 导入pandas库，用于数据处理和分析")
      st.write("from sklearn.model_selection import train_test_split  # 导入数据划分函数")
      st.write("读取CSV文件")
      st.write("data = pd.read_csv('iris.csv')  # 从当前目录读取名为'iris.csv'的文件到DataFrame")
      st.write("检查数据前几行，确保正确读取")
      st.write("print(data.head())  # 打印数据的前5行，用于预览数据结构和内容")
      st.write("代码功能：")
      st.write("导入必要库：pandas用于数据处理，train_test_split用于后续的数据集划分")
      st.write("读取数据：从CSV文件加载数据到pandas DataFrame")
      st.write("数据预览：显示前几行数据，检查数据是否正确加载，查看列名和数据类型")
    ...
    if st.button("解释一下以上代码"):
      ...
      st.write("#读取TXT格式的鸢尾花数据")
      st.write("data = pd.read_csv('iris.txt', sep='\s+', header=0)")
      st.write("使用pd.read_csv读取TXT文件（CSV和TXT都是文本格式）")
      st.write("sep='\s+'：使用正则表达式匹配一个或多个空格/制表符作为分隔符")
      st.write("header=0：将第一行作为列名")
      st.write("#提取特征数据")
      st.write("features = data.iloc[:, :4].astype(float).values")
      st.write("data.iloc[:, :4]：选择所有行的前4列（特征列）")
      st.write(".astype(float)：将数据类型转换为浮点数")
      st.write(".values：转换为NumPy数组格式")
      st.write("#提取并转换标签数据")
      st.write("species_mapping = {'setosa':0, 'versicolor':1, 'virginica':2}")
      st.write("labels = data['Species'].map(species_mapping).astype(int).values")
      st.write("创建字典将文本标签映射为数字")
      st.write("data['Species']：选择名为'Species'的标签列")
      st.write(".map(species_mapping)：应用映射关系将文本转为数字")
      st.write(".astype(int).values：转为整数类型的NumPy数组")
      st.write("#划分训练集和测试集")
      st.write("from sklearn.model_selection import train_test_split")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("features, labels, test_size=0.2, random_state=42")
      st.write(")")
      st.write("test_size=0.2：20%的数据作为测试集，80%作为训练集")
      st.write("random_state=42：设置随机种子，确保每次划分结果相同")
  ...
  # 页面5：模型训练
    ...
    st.subheader("🎲 推荐问题")
    if st.button("帮我解释一下上述代码"):
       st.write("#导入K近邻分类器")
       st.write("from sklearn.neighbors import KNeighborsClassifier")
       st.write("#创建KNN分类器实例")
       st.write("clf_KNN = KNeighborsClassifier(n_neighbors=5)")
       st.write("KNeighborsClassifier：scikit-learn中的K近邻分类器类")
       st.write("n_neighbors=5：设置K值为5，即考虑最近的5个邻居来进行分类")
       st.write("#训练模型")
       st.write("clf_KNN.fit(X_train, Y_train)")
       st.write(".fit()：训练方法，用训练数据来建立模型")
       st.write("X_train：训练集特征数据（花的测量值）")
       st.write("Y_train：训练集标签数据（花的种类）")
    if st.button("为什么n_neighbors选择5？"):
       st.write("选择K=5是经验性的初始选择，但并不是固定不变的：")
       st.write("选择K=5的考虑：")
       st.write("平衡考虑：")
       st.write("K太小（如K=1）：模型太敏感，容易受噪声影响，可能过拟合")
       st.write("K太大（如K=50）：模型太简单，可能忽略局部特征，导致欠拟合")
       st.write("经验法则：")
       st.write("对于150个样本的数据集，K=5是一个合理的起点")
       st.write("通常K值取奇数，避免平票情况")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("以上代码是什么意思？"):
       st.write("#导入准确率计算函数")
       st.write("from sklearn.metrics import accuracy_score")
       st.write("accuracy_score是scikit-learn中专门用于计算分类准确率的函数")
       st.write("#计算KNN模型的准确率")
       st.write("acc_KNN = accuracy_score(Y_test, KNN_pred)")
       st.write("Y_test：测试集的真实标签（实际正确的类别）")
       st.write("KNN_pred：模型对测试集的预测标签（模型预测的类别）")
       st.write("accuracy_score会比较这两个数组，计算预测正确的比例")
       st.write("#打印准确率结果")
       st.write("print('KNN的准确率:', round(acc_KNN, 2))")
       st.write("round(acc_KNN, 2)：将准确率四舍五入保留2位小数")
       st.write("例如：0.966666... → 0.97")
       st.write("准确率计算公式")
       st.write("准确率 = (预测正确的样本数) / (总测试样本数) × 100%")
    st.subheader("[查看代码运行结果]")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("请帮我解释一下上述代码"):
       st.write("#初始化正确预测计数器")
       st.write("accnum_KNN = 0")
       st.write("创建一个变量accnum_KNN，初始值为0，用于统计预测正确的样本数量")
       st.write("#遍历测试集中的每一个样本")
       st.write("for i in range(Y_test.shape[0]):")
       st.write("Y_test.shape[0]：获取测试集样本的总数")
       st.write("range(Y_test.shape[0])：生成从0到测试集样本数-1的索引序列")
       st.write("循环会遍历测试集中的每一个样本")
       st.write("#判断预测是否正确")
       st.write("if KNN_pred[i] == Y_test[i]:")
       st.write(" #如果预测正确，计数器加1")
       st.write(" accnum_KNN = accnum_KNN + 1")
       st.write("KNN_pred[i]：模型对第i个样本的预测结果")
       st.write("Y_test[i]：第i个样本的真实标签")
       st.write("如果预测值与真实值相等，说明预测正确，计数器加1")
       st.write("#计算并打印准确率")
       st.write("print('KNN的准确率:', round(accnum_KNN / Y_test.shape[0], 2))")
       st.write("accnum_KNN / Y_test.shape[0]：正确预测数 ÷ 总样本数 = 准确率")
       st.write("round(..., 2)：将结果四舍五入保留2位小数")
    ...
    if st.button("讲讲上述代码"):
      st.write("#导入混淆矩阵函数")
      st.write("from sklearn.metrics import confusion_matrix")
      st.write("#计算混淆矩阵")
      st.write("KNN_matrix = confusion_matrix(Y_test, KNN_pred)")
      st.write("confusion_matrix是用于评估分类模型性能的重要工具")
      st.write("Y_test：测试集的真实标签")
      st.write("KNN_pred：模型的预测标签")
      st.write("函数会比较这两个数组，生成混淆矩阵")
      st.write("#打印文本但不换行")
      st.write("print('KNN的混淆矩阵为：', end="")")
      st.write("end=""：指定print函数的结束符为空字符串（默认为换行符\n）")
      st.write("这样下一个print输出会接在同一行后面")
      st.write("#打印一个空行（实际是换行）")
      st.write("print()")
      st.write("这个空的print语句会输出一个换行符")
      st.write("目的是让混淆矩阵在下一行显示，使输出更清晰")
      st.write("#打印混淆矩阵")
      st.write("print(KNN_matrix)")
      st.write("直接输出混淆矩阵的内容")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("以上这段代码是什么意思"):
      st.write("#计算混淆矩阵的行和（每个真实类别的总样本数）")
      st.write("row_sums = np.sum(KNN_matrix, axis=1)")
      st.write("axis=1：沿着水平方向（行方向）求和")
      st.write("对每一行分别求和，得到每个真实类别的总样本数")
      st.write("结果是一个一维数组，长度等于类别数量")
      st.write("#计算混淆矩阵的列和（每个预测类别的总样本数）")
      st.write("colm_sums = np.sum(KNN_matrix, axis=0)")
      st.write("axis=0：沿着垂直方向（列方向）求和")
      st.write("对每一列分别求和，得到每个预测类别的总样本数")
      st.write("结果是一个一维数组，长度等于类别数量")
 ...
 # 页面6：模型训练
    ...
    st_highlight("from sklearn.datasets import load_iris")
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.preprocessing import StandardScaler")
    st_highlight("from sklearn.neighbors import KNeighborsClassifier")
    st_highlight("from sklearn.metrics import accuracy_score")
  ...
  # 页面7：模型训练
    ...
    st_highlight("#%%")
    st_highlight("import pandas as pd")
    st_highlight("from sklearn.tree import DecisionTreeClassifier,export_text,plot_tree")
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.preprocessing import LabelEncoder")
    st_highlight("from sklearn.metrics import classification_report,confusion_matrix,accuracy_score")
    st_highlight("import matplotlib.pyplotasplt")
    st_highlight("#1.加载数据")
    st_highlight("from sklearn import datasets")
    st_highlight("iris_datas=datasets.load_iris()")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("分析这段代码的意思"):
      st.write("1. 导入必要的库")
      st.write("import pandas as pd")
      st.write("from sklearn.tree import DecisionTreeClassifier, export_text, plot_tree")
      st.write("from sklearn.model_selection import train_test_split")
      st.write("from sklearn.preprocessing import LabelEncoder")
      st.write("from sklearn.metrics import classification_report, confusion_matrix, accuracy_score")
      st.write("import matplotlib.pyplot as plt")
      st.write("导入了数据处理、决策树模型、数据划分、预处理、评估指标和可视化所需的库")
      st.write("2. 加载数据")
      st.write("from sklearn import datasets")
      st.write("iris_datas = datasets.load_iris()")
      st.write("从sklearn内置数据集中加载鸢尾花数据集")
      st.write("3. 分离特征和标签")
      st.write("feature = iris_datas.data")
      st.write("label = iris_datas.target")
      st.write("feature：包含4个特征（花萼长度、花萼宽度、花瓣长度、花瓣宽度）")
      st.write("label：包含3个类别标签（0:setosa, 1:versicolor, 2:virginica）")
      st.write("4. 划分训练集和测试集")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("    feature, label, test_size=0.2, random_state=42")
      st.write(")")
      st.write('print("\n训练集特征形状:", X_train.shape, "测试集特征形状:", X_test.shape)')
      st.write("将数据按80%训练集、20%测试集的比例划分")
      st.write("random_state=42：确保每次划分结果相同")
      st.write("打印数据形状确认划分正确（训练集120个样本，测试集30个样本）")
      st.write("5. 创建决策树模型")
      st.write("dt_model = DecisionTreeClassifier(")
      st.write("    criterion='gini',      # 分裂标准：基尼系数")
      st.write("    max_depth=3,           # 树的最大深度")
      st.write("    min_samples_split=2,   # 节点分裂所需最小样本数")
      st.write("    random_state=42        # 随机种子")
      st.write(")")
      st.write("参数解释：")
      st.write("criterion='gini'：使用基尼不纯度作为分裂标准（也可以选'entropy'信息增益）")
      st.write("max_depth=3：限制树的最大深度为3层，防止过拟合")
      st.write("min_samples_split=2：节点至少需要2个样本才会继续分裂")
      st.write("random_state=42：确保每次运行结果一致")
      st.write("6. 训练决策树模型")
      st.write("dt_model.fit(X_train, y_train)")
      st.write("使用训练数据来构建决策树模型")
    ...
    st_highlight("print('\n特征重要性:\n',importance)")
    st.subheader("🎲 推荐问题")
    if st.button("帮我分析这段代码是什么意思"):
      st.write("1. 可视化决策树")
      st.write("plt.figure(figsize=(15,10))")
      st.write("plot_tree(")
      st.write("    dt_model,")
      st.write("    feature_names=iris_datas.feature_names,  # 使用数据集自带的特征名称")
      st.write("    class_names=iris_datas.target_names,     # 使用类别名称而不是数字")
      st.write("    filled=True,                             # 用颜色填充节点")
      st.write("    rounded=True                             # 节点使用圆角")
      st.write(")")
      st.write('plt.title("Decision Tree Visualization")')
      st.write("plt.show()")
      st.write("参数说明：")
      st.write('feature_names：显示特征的实际名称（如"sepal length"）')
      st.write('class_names：显示类别的实际名称（如"setosa"）')
      st.write("filled=True：根据类别用不同颜色填充节点，便于区分")
      st.write("rounded=True：节点显示为圆角，视觉效果更好")
      st.write("可视化结果包含的信息：")
      st.write("每个节点的分裂条件和阈值")
      st.write("当前节点的基尼不纯度")
      st.write("样本数量分布")
      st.write("类别分布比例")
      st.write("2. 输出决策规则")
      st.write("以文本形式展示决策树的完整逻辑")
      st.write("可以清晰地看到分类的决策路径")
      st.write("便于理解和解释模型的决策过程")
      st.write("3. 特征重要性分析")
      st.write("特征重要性说明：")
      st.write("决策树会计算每个特征在分类中的重要性")
      st.write("重要性基于特征在分裂节点时减少不纯度的能力")
      st.write("数值范围：0到1，所有特征重要性之和为1")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("请解释这段代码是什么意思"):
      st.write("1. 选择最优特征")
      st.write("X_train_best = X_train[:, -2:]  # 选取最后两个特征")
      st.write("X_test_best = X_test[:, -2:]    # 选取最后两个特征")
      st.write("X_train[:, -2:]：从训练集中选取最后两列（所有行的最后两个特征）")
      st.write("X_test[:, -2:]：从测试集中选取最后两列")
      st.write("基于之前的特征重要性分析，最后两个特征（花瓣长度和花瓣宽度）是最重要的")
      st.write("2. 创建新的决策树模型")
      st.write("dt_model_best = DecisionTreeClassifier(")
      st.write("    criterion='gini',           # 分裂标准：基尼系数")
      st.write("    max_depth=3,                # 树的最大深度")
      st.write("    min_samples_split=2,        # 节点分裂所需最小样本数")
      st.write("    random_state=42             # 随机种子")
      st.write(")")
      st.write("dt_model_best.fit(X_train_best, y_train)")
      st.write("使用与原始模型相同的参数配置")
      st.write("但只使用最重要的两个特征进行训练")
      st.write("3. 使用最优特征模型进行预测")
      st.write("y_pred_best = dt_model_best.predict(X_test_best)")
      st.write('print("\n混淆矩阵:")')
      st.write("print(confusion_matrix(y_test, y_pred))")
      st.write('print("\n准确率:", accuracy_score(y_test, y_pred))')
      st.write("#应该打印最优特征模型的结果")
      st.write('print("\n最优特征模型混淆矩阵:")')
      st.write("print(confusion_matrix(y_test, y_pred_best))")
      st.write('print("\n最优特征模型准确率:", accuracy_score(y_test, y_pred_best))')
    if st.button("为什么选择最后两个特征？"): 
      st.write("基于之前的特征重要性分析：")
      st.write("花瓣长度 (petal length)：最重要特征")
      st.write("花瓣宽度 (petal width)：次重要特征")
      st.write("花萼相关特征：重要性接近0")
    if st.button("特征选择的好处是什么？"):
      st.write("减少过拟合：使用更少的特征可以降低模型复杂度")
      st.write("提高训练速度：计算量减少")
      st.write("可能提高泛化能力：去除噪声特征")
  ...
  # 页面8：模型训练
    ...
    st.subheader("🎲 推荐问题")
    if st.button("解释这段代码是什么意思"):
      st.write("#导入支持向量机分类器")
      st.write("from sklearn.svm import SVC")
      st.write("SVC：Support Vector Classification，是sklearn中用于分类的支持向量机")
      st.write("#创建SVM分类器实例")
      st.write("clf_SVM = SVC(kernel='linear')")
      st.write("kernel='linear'：指定使用线性核函数")
      st.write("这会创建一个线性支持向量机分类器")
      st.write("#训练SVM模型")
      st.write("clf_SVM.fit(X_train, Y_train)")
      st.write("使用训练数据来训练SVM模型")
      st.write("X_train：训练集特征")
      st.write("Y_train：训练集标签")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("请解释上述代码是什么意思"):
      st.write("#使用训练好的SVM模型对测试集进行预测")
      st.write("SVM_pred = clf_SVM.predict(X_test)")
      st.write("clf_SVM：之前训练好的支持向量机模型")
      st.write("predict(X_test)：对测试集特征数据进行预测")
      st.write("SVM_pred：存储模型预测结果的变量")
      st.write("#计算准确率")
      st.write("from sklearn.metrics import accuracy_score")
      st.write("acc_SVM = accuracy_score(y_test, SVM_pred)")
      st.write("accuracy_score：计算分类准确率的函数")
      st.write("y_test：测试集的真实标签（正确答案）")
      st.write("SVM_pred：模型的预测结果")
      st.write("acc_SVM：存储准确率结果的变量")
      st.write("#格式化输出准确率")
      st.write("print('SVM的准确率:{:.2%}'.format(acc_SVM))")
      st.write("{:.2%}：格式化字符串，将数字转换为百分比格式并保留2位小数")
      st.write(".format(acc_SVM)：将准确率值插入到格式化字符串中")
      st.write("输出示例：")
      st.write("如果准确率为0.9667，输出将是：")
      st.write("SVM的准确率:96.67%")
      st.write("代码分析：")
      st.write("1. 预测过程")
      st.write("模型使用学习到的决策边界对新的未知数据（X_test）进行分类")
      st.write("对于每个测试样本，输出一个预测的类别标签")
      st.write("2. 准确率计算")
      st.write("准确率 = (正确预测的样本数) / (总测试样本数)")
      st.write("比较预测结果与真实标签的一致性")
      st.write("返回一个0到1之间的值，表示模型整体的分类正确率")
      st.write("3. 格式化输出的优势")
      st.write("百分比格式更直观易懂")
      st.write("保留2位小数提供适当的精度")
      st.write("适合在报告或演示中展示结果")
   ...
   # 按钮4：模型预测与评估
    ...
    st_highlight("#导入所需库")
    st_highlight("import numpy as np")
    st_highlight("import matplotlib.pyplot as plt")
    st_highlight("from sklearn import datasets")
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.svm import SVC")
    st_highlight("from sklearn.metrics import classification_report,confusion_matrix")
    st_highlight("from sklearn.preprocessing import StandardScaler")
    st_highlight("#1.加载数据")
    st_highlight("iris_datas=datasets.load_iris()")
    ...
    st_highlight("print(confusion_matrix(y_test,y_pred))")
    st.subheader("🎲 推荐问题")
    if st.button("请给我讲讲这段代码是什么意思"):
      st.write("导入所需库：")
      st.write("导入了数据处理（numpy）、可视化（matplotlib）、数据集（sklearn.datasets）等工具")
      st.write("导入了 SVM 分类器（SVC）、模型评估指标和数据预处理工具")
      st.write("加载数据：")
      st.write("iris_datas = datasets.load_iris()")
      st.write("feature = iris_datas.data  # 特征数据（花萼长度、花萼宽度、花瓣长度、花瓣宽度）")
      st.write("label = iris_datas.target  # 标签数据（0、1、2分别代表三种鸢尾花）")
      st.write("target_names1 = iris_datas.target_names  # 类别名称（setosa、versicolor、virginica）")
      st.write("feature_names = iris_datas.feature_names  # 特征名称")
      st.write("加载经典的鸢尾花数据集，包含 3 个类别的鸢尾花，每个类别有 4 个特征。")
      st.write("数据预处理：")
      st.write("scaler = StandardScaler()")
      st.write("feature_scaled = scaler.fit_transform(feature)")
      st.write("对特征进行标准化处理，将特征缩放到均值为 0、标准差为 1 的范围内。这对 SVM 非常重要，因为 SVM 对特征的尺度敏感。")
      st.write("划分训练集和测试集：")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("    feature_scaled, label, test_size=0.2, random_state=42")
      st.write(")")
      st.write("将数据集划分为训练集（80%）和测试集（20%），random_state=42确保结果可重现。")
      st.write("创建带 RBF 核的 SVM 模型：")
      st.write("svm_model = SVC(")
      st.write("    kernel='rbf',  # 使用径向基函数核（适合处理非线性问题）")
      st.write("    C=1.0,  # 正则化参数，控制容错能力，值越大容错能力越弱")
      st.write("    gamma='scale',  # 核系数，自动根据特征数量调整")
      st.write("    probability=True,  # 启用概率估计（可以预测属于每个类别的概率）")
      st.write("    random_state=42  # 随机种子，确保结果可重现")
      st.write(")")
      st.write("训练模型：")
      st.write("svm_model.fit(X_train, y_train)")
      st.write("使用训练集数据训练 SVM 模型。")
      st.write("模型评估：")
      st.write("y_pred = svm_model.predict(X_test)：使用训练好的模型对测试集进行预测")
      st.write("y_prob = svm_model.predict_proba(X_test)：获取每个测试样本属于各个类别的概率")
      st.write("输出评估结果：")
      st.write("分类性能报告：包含每个类别的精确率（precision）、召回率（recall）、F1 分数和支持数")
      st.write("混淆矩阵：展示预测类别与实际类别的对应关系，对角线元素表示预测正确的数量")
    ...
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.preprocessing import StandardScaler")
    st_highlight("from sklearn.metrics import accuracy_score,classification_report")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("请解释上述代码的意思"):
      st.write("导入必要的库：")
      st.write("numpy：用于数值计算")
      st.write("sklearn.datasets：提供鸢尾花数据集")
      st.write("sklearn.svm.SVC：SVM 分类器")
      st.write("数据划分、标准化和评估相关工具")
      st.write("加载和准备数据：")
      st.write("#加载鸢尾花数据集")
      st.write("iris = datasets.load_iris()")
      st.write("#为了简化，只取前两个特征")
      st.write("X = iris.data[:, :2]")
      st.write("y = iris.target")
      st.write("这里使用了鸢尾花数据集，为了后续可能的可视化等操作，只选取了前两个特征。")
      st.write("数据划分：")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("    X, y, test_size=0.3, random_state=42")
      st.write(")")
      st.write("将数据集划分为训练集 (70%) 和测试集 (30%)，random_state=42确保结果可重现。")
      st.write("数据标准化：")
      st.write("scaler = StandardScaler()")
      st.write("X_train = scaler.fit_transform(X_train)  # 拟合并转换训练集")
      st.write("X_test = scaler.transform(X_test)        # 用相同的缩放规则转换测试集")
      st.write("SVM 对特征尺度敏感，因此需要标准化处理，将特征缩放到均值为 0、方差为 1 的范围内。")
      st.write("创建不同核函数的 SVM 模型：")
      st.write("rbf：径向基函数核（默认核函数）")
      st.write("linear：线性核函数")
      st.write("poly：多项式核函数")
      st.write("sigmoid：Sigmoid 核函数")
      st.write("每个模型都有其特定参数，如多项式核的degree表示多项式阶数。")
      st.write("模型训练与评估：")
      st.write("for kernel, name in zip(kernels, kernel_names):")
      st.write("    kernel.fit(X_train, y_train)          # 训练模型")
      st.write("    y_pred = kernel.predict(X_test)       # 预测")
      st.write("    accuracy = accuracy_score(y_test, y_pred)  # 计算准确率")
      st.write("循环遍历每个模型，进行训练和预测，并计算准确率。")
      st.write("输出评估结果：")
      st.write("打印每个核函数的准确率")
      st.write("输出详细的分类报告，包括精确率 (precision)、召回率 (recall)、F1 分数等")
    st.subheader("[查看代码运行结果]")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("这些代码是什么意思"):
      st.write("首先定义了一个plot_2d_decision_boundary函数，它接收四个参数：")
      st.write("model：要可视化的机器学习模型")
      st.write("X：特征数据")
      st.write("y：标签数据")
      st.write("feature_names：特征名称列表，用于图表标注")
      st.write("函数内部操作：")
      st.write("X = X[:, :2]  # 只使用前两个特征进行可视化")
      st.write("model.fit(X, y)  # 使用这两个特征重新训练模型")
      st.write("因为要可视化二维决策边界，所以只保留前两个特征，并使用这两个特征重新训练模型。")
      st.write("创建网格点用于绘制决策边界：")
      st.write("x_min, x_max = X[:, 0].min() - 1, X[:, 0].max() + 1")
      st.write("y_min, y_max = X[:, 1].min() - 1, X[:, 1].max() + 1")
      st.write("xx, yy = np.meshgrid(np.arange(x_min, x_max, 0.02),  np.arange(y_min, y_max, 0.02))")
      st.write("这段代码确定了第一个特征 (x 轴) 和第二个特征 (y 轴) 的范围，并创建了一个细密的网格，后续会对网格上的每个点进行预测。")
      st.write("预测网格点并绘制决策边界：")
      st.write("Z = model.predict(np.c_[xx.ravel(), yy.ravel()])  # 对所有网格点进行预测")
      st.write("Z = Z.reshape(xx.shape)  # 将预测结果重塑为网格形状")
      st.write("plt.contourf(xx, yy, Z, alpha=0.4)  # 绘制决策区域，不同类别用不同颜色表示")
      st.write("plt.scatter(X[:, 0], X[:, 1], c=y, s=20, edgecolor='k')  # 绘制原始数据点")
      st.write("设置图表标签和标题：")
      st.write("plt.xlabel(feature_names[0])  # x轴使用第一个特征的名称")
      st.write("plt.ylabel(feature_names[1])  # y轴使用第二个特征的名称")
      st.write('plt.title("SVM决策边界(基于前两个特征)")  # 设置图表标题')
      st.write("最后调用该函数进行可视化：")
      st.write("plt.figure(figsize=(10,6))  # 创建一个指定大小的图表")
      st.write("plot_2d_decision_boundary(svm_model, feature_scaled, label, feature_names)")
      st.write("plt.show()  # 显示图表")
      st.write("运行这段代码后，你会得到一个二维图表，其中：")
      st.write("不同颜色的区域代表模型对不同类别的预测")
      st.write("散点表示原始数据点，颜色代表它们的真实类别")
      st.write("区域之间的边界就是模型的决策边界")
  # 页面9：模型训练
   elif page == "模型4:朴素贝叶斯":
    st.title("模型4 朴素贝叶斯")
    ...
    st_highlight("from sklearn import datasets")
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.naive_bayes import GaussianNB")
    st_highlight("from sklearn.metrics import classification_report,confusion_matrix,accuracy_score")
    st_highlight("from sklearn.preprocessing import StandardScaler")
    ...
    st.subheader("🎲 推荐问题")
    if st.button("分析上述这段代码是什么意思"):
      st.write("导入所需库：")
      st.write("导入了数据处理（numpy）、数据集（sklearn.datasets）等工具")
      st.write("导入了高斯朴素贝叶斯分类器（GaussianNB）、模型评估指标和数据预处理工具")
      st.write("加载数据：")
      st.write("iris_datas = datasets.load_iris()")
      st.write("feature = iris_datas.data  # 特征数据（花萼长度、花萼宽度、花瓣长度、花瓣宽度）")
      st.write("label = iris_datas.target  # 标签数据（0、1、2分别代表三种鸢尾花）")
      st.write("target_names = iris_datas.target_names  # 类别名称（setosa、versicolor、virginica）")
      st.write("feature_names = iris_datas.feature_names  # 特征名称")
      st.write("加载经典的鸢尾花数据集，包含 3 个类别的鸢尾花，每个类别有 4 个特征。")
      st.write("数据预处理：")
      st.write("scaler = StandardScaler()")
      st.write("feature_scaled = scaler.fit_transform(feature)")
      st.write("对特征进行标准化处理，将特征缩放到均值为 0、标准差为 1 的范围内。虽然朴素贝叶斯对特征尺度不如 SVM 敏感，但标准化可以让不同特征具有可比性。")
      st.write("划分训练集和测试集：")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("    feature_scaled, label, test_size=0.2, random_state=42")
      st.write(")")
      st.write("将数据集划分为训练集（80%）和测试集（20%），random_state=42确保结果可重现。")
      st.write("创建高斯朴素贝叶斯分类器：")
      st.write("clf_NB = GaussianNB()")
      st.write("高斯朴素贝叶斯是朴素贝叶斯的一种变体，假设特征服从高斯（正态）分布，适用于连续型特征数据。")
      st.write("训练模型：")
      st.write("clf_NB.fit(X_train, y_train)")
      st.write("使用训练集数据训练朴素贝叶斯模型，模型会学习每个类别的先验概率和每个特征在不同类别下的高斯分布参数（均值和方差）。")
      st.write("模型评估：")
      st.write("y_pred = clf_NB.predict(X_test)：使用训练好的模型对测试集进行预测")
      st.write("输出评估结果：")
      st.write("分类报告：包含每个类别的精确率（precision）、召回率（recall）、F1 分数和支持数")
      st.write("混淆矩阵：展示预测类别与实际类别的对应关系，对角线元素表示预测正确的数量")
      st.write("准确率：整体预测正确的比例")
      st.write("输出先验概率：")
      st.write('print("\n各类别先验概率:", clf_NB.class_prior_)')
      st.write("输出每个类别的先验概率，即训练集中每个类别出现的频率（或概率），这是朴素贝叶斯算法的重要参数。")
      st.write("输出预测概率：")
      st.write('print("\n测试集前5个样本的预测概率:")')
      st.write("print(clf_NB.predict_proba(X_test[:5]))")
      st.write("输出测试集中前 5 个样本属于每个类别的概率，每行的概率和为 1，模型会选择概率最高的类别作为预测结果。")
    st.subheader("[查看代码运行结果]")
    st.title("🌸 朴素贝叶斯分类器 ")
    if st.button("1. 加载数据"):
     iris_datas = datasets.load_iris()
  ...
  # 页面10：模型训练
    ...
    st_highlight("from sklearn import datasets")
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.neural_network import MLPClassifier")
    st_highlight("from sklearn.metrics import classification_report,confusion_matrix")
    st_highlight("from sklearn.preprocessing import StandardScaler")
    st_highlight("#1.加载数据（使用指定变量名）")
    st_highlight("iris_datas=datasets.load_iris()")
    ...
    st_highlight("plt.show()")
    st.subheader("🎲 推荐问题")
    if st.button("说说这段代码是什么意思"):
      st.write("导入所需库：")
      st.write("包含数据处理（numpy）、数据集加载、模型训练与评估等工具")
      st.write("重点使用了MLPClassifier，这是 scikit-learn 中的多层感知器分类器")
      st.write("加载数据：")
      st.write("iris_datas = datasets.load_iris()")
      st.write("feature = iris_datas.data  # 特征数据（4个鸢尾花特征）")
      st.write("label = iris_datas.target  # 标签数据（0、1、2代表三种鸢尾花）")
      st.write("target_names2 = iris_datas.target_names  # 类别名称")
      st.write("加载经典的鸢尾花数据集，用于后续的分类任务。")
      st.write("数据预处理：")
      st.write("scaler = StandardScaler()")
      st.write("feature_scaled = scaler.fit_transform(feature)")
      st.write("对特征进行标准化处理，神经网络对输入特征的尺度较为敏感，标准化能帮助模型更好地收敛。")
      st.write("划分训练集和测试集：")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("    feature_scaled, label, test_size=0.2, random_state=42")
      st.write(")")
      st.write("按 8:2 比例划分训练集和测试集，random_state=42确保结果可重现。")
      st.write("创建 MLP 模型：")
      st.write("clf_MLP = MLPClassifier(")
      st.write("    hidden_layer_sizes=(10, 10),  # 两个隐藏层，每个包含10个神经元")
      st.write("    activation='relu',  # 使用ReLU激活函数，处理非线性关系")
      st.write("    solver='adam',  # Adam优化器，高效的梯度下降算法")
      st.write("    alpha=0.01,  # L2正则化参数，防止过拟合")
      st.write("    learning_rate_init=0.01,  # 初始学习率")
      st.write("    max_iter=200,  # 最大迭代次数（epochs）")
      st.write("    early_stopping=True  # 启用早停策略，防止过拟合")
      st.write(")")
      st.write("这里定义了一个两层隐藏层的神经网络，是代码的核心部分。")
      st.write("训练模型：")
      st.write("clf_MLP.fit(X_train, y_train)")
      st.write("使用训练数据拟合模型，神经网络通过反向传播算法学习特征与标签之间的关系。")
      st.write("模型预测：")
      st.write("y_pred = clf_MLP.predict(X_test)：对测试集进行预测，得到分类结果")
      st.write("y_prob = clf_MLP.predict_proba(X_test)：获取每个样本属于各类别的概率")
      st.write("输出结果：")
      st.write("打印真实标签与预测标签的对比")
      st.write("详细展示每个测试样本的预测概率分布")
      st.write("输出分类报告（精确率、召回率、F1 分数等）")
      st.write("输出混淆矩阵，展示分类错误情况")
      st.write("绘制训练损失曲线：")
      st.write("plt.plot(mlp.loss_curve_)")
      st.write('plt.title("MLP训练损失曲线")')
      st.write('plt.xlabel("迭代次数")')
      st.write('plt.ylabel("损失值")')
      st.write("plt.show()")
      st.write("可视化训练过程中的损失变化，损失值通常会随着训练迭代逐渐下降并趋于稳定，可用于判断模型是否收敛。")
    st.subheader("[查看代码运行结果]")
    st.title("🤖 MLP 分类器实验 ")
    if st.button("1. 加载数据"):
     iris_datas = datasets.load_iris()
    ...
    st.subheader("🎲 推荐问题")
    if st.button("给我解释上述这段代码的意思"):
      st.write("导入库与加载数据：与之前的代码类似，导入了必要的库并加载了鸢尾花数据集，包含特征数据、标签、类别名称等。")
      st.write("数据预处理与划分：")
      st.write("对特征进行标准化处理，确保神经网络能更好地收敛")
      st.write("将数据集按 8:2 划分为训练集和测试集，保持结果可重现性")
      st.write("创建 MLP 模型（关键配置）：")
      st.write("clf_MLP = MLPClassifier(")
      st.write("    hidden_layer_sizes=(100,),  # 1个隐藏层，包含100个神经元")
      st.write("    activation='relu',          # ReLU激活函数")
      st.write("    solver='adam',              # Adam优化器")
      st.write("    max_iter=1,                 # 每次只迭代1次（配合自定义循环）")
      st.write("    warm_start=True,            # 保留模型状态，允许继续训练")
      st.write("    early_stopping=False        # 关闭早停，完整记录训练过程")
      st.write(")")
      st.write("这里的关键设置是max_iter=1和warm_start=True，这使得我们可以手动控制训练轮数，而不是让模型自动完成所有迭代。")
      st.write("自定义训练循环（核心部分）：")
      st.write("epochs = 200  # 总训练轮数")
      st.write("train_losses = []  # 存储训练损失")
      st.write("test_losses = []   # 存储测试损失")
      st.write("for epoch in range(epochs):")
      st.write("    #训练一个迭代并记录训练损失")
      st.write("    clf_MLP.fit(X_train, y_train)")
      st.write("    train_losses.append(clf_MLP.loss_)")
      st.write("    #计算并记录测试损失")
      st.write("   y_pred_proba = clf_MLP.predict_proba(X_test)")
      st.write("   #将真实标签转换为独热编码")
      st.write("    y_test_onehot = np.zeros((y_test.size, y_test.max()+1))")
      st.write("    y_test_onehot[np.arange(y_test.size), y_test] = 1")
      st.write("   #计算交叉熵损失")
      st.write("    test_loss = -np.mean(np.sum(y_test_onehot * np.log(y_pred_proba + 1e-10), axis=1))")
      st.write("    test_losses.append(test_loss)")
      st.write("这段代码实现了：")
      st.write("手动控制 200 轮训练过程")
      st.write("每轮训练后记录训练损失（直接从模型获取）")
      st.write("手动计算并记录测试损失（通过交叉熵公式）")
      st.write("每 20 轮打印一次损失情况，监控训练进度")
      st.write("模型评估与结果输出：")
      st.write("对测试集进行预测并生成分类报告")
      st.write("分类报告包含精确率、召回率、F1 分数等关键指标")
      st.write("损失曲线可视化：")
      st.write("plt.plot(range(1, epochs+1), train_losses, label='Train Loss')")
      st.write("plt.plot(range(1, epochs+1), test_losses, label='Test Loss')")
      st.write("绘制了训练集和测试集的损失曲线，通过对比两条曲线可以：")
      st.write("判断模型是否收敛（损失是否趋于稳定）")
      st.write("检测过拟合现象（测试损失开始上升而训练损失持续下降）")
      st.write("分析模型的学习效果和泛化能力")
    st.subheader("[查看代码运行结果]")
 ...
 # 8. 绘制训练/测试损失曲线
  ...
  # 页面11：模型训练
    ...
    st_highlight("import matplotlib.pyplot as plt")
    st_highlight("from sklearn.datasets import load_iris")
    st_highlight("from sklearn.ensemble import RandomForestClassifier")
    st_highlight("from sklearn.model_selection import train_test_split")
    st_highlight("from sklearn.metrics import classification_report,accuracy_score")
    st_highlight("#1.加载数据")
    ...
    if st.button("这段代码是什么意思？"):
      st.write("导入所需库：")
      st.write("包含数据处理（pandas、numpy）、可视化（matplotlib）、数据集加载、随机森林模型和评估工具")
      st.write("加载数据：")
      st.write("iris_datas = load_iris()")
      st.write("feature = iris_datas.data  # 特征数据（4个鸢尾花特征）")
      st.write("label = iris_datas.target  # 标签数据（0、1、2代表三种鸢尾花）")
      st.write("feature_names = iris.feature_names  # 特征名称（花萼长度、宽度等）")
      st.write("target_names = iris.target_names  # 类别名称")
      st.write("加载经典的鸢尾花数据集，用于分类任务。")
      st.write("划分训练集和测试集：")
      st.write("X_train, X_test, y_train, y_test = train_test_split(")
      st.write("    feature, label, test_size=0.2, random_state=42")
      st.write(")")
      st.write("按 8:2 比例划分训练集和测试集，random_state=42确保结果可重现。")
      st.write("创建并训练随机森林模型：")
      st.write("clf_RF = RandomForestClassifier(")
      st.write("    n_estimators=100,  # 森林中包含100棵决策树")
      st.write("    random_state=42,")
      st.write("    max_depth=3        # 限制每棵树的最大深度，防止过拟合")
      st.write(")")
      st.write("clf_RF.fit(X_train, y_train)")
      st.write("随机森林是一种集成学习方法，通过构建多棵决策树并综合它们的预测结果来提高性能。")
      st.write("模型评估：")
      st.write("y_pred = clf_RF.predict(X_test)")
      st.write('print("准确率:", accuracy_score(y_test, y_pred))')
      st.write('print("\n分类报告:")')
      st.write('print(classification_report(y_test, y_pred, target_names=target_names))')
      st.write("计算并输出模型准确率")
      st.write("生成详细分类报告，包含每个类别的精确率、召回率、F1 分数等")
      st.write("特征重要度分析：")
      st.write("importances = clf_RF.feature_importances_  # 获取特征重要度")
      st.write("std = np.std([tree.feature_importances_ for tree in rf.estimators_], axis=0)  # 计算标准差")
      st.write("随机森林的一大优势是可以直接输出特征重要度，表明每个特征对分类决策的贡献程度。")
      st.write("特征重要度展示：")
      st.write("将特征重要度整理成 DataFrame 并按重要性排序")
      st.write("打印排序后的特征重要度，方便查看具体数值")
      st.write("特征重要度可视化：")
      st.write("plt.bar(")
      st.write("    range(feature.shape[1]),")
      st.write("    importances,")
      st.write("    yerr=std,")
      st.write(' align="center",' )
      st.write("    color='lightblue',")
      st.write("    ecolor='black'")
      st.write(")")
      st.write("绘制特征重要度条形图，其中：")
      st.write("条形高度表示特征重要度")
      st.write("误差线表示不同决策树之间特征重要度的标准差")
      st.write("x 轴显示特征名称，按重要性排序")
    st.subheader("[查看代码运行结果]")
    st.title("🌳 随机森林分类实验 ")
 ...
 # 6. 可视化特征重要度
    ...
    st.subheader("🎲 推荐问题")
    if st.button("这段代码是什么意思呢？"):
      st.write("（1）标签变量赋值")
      st.write("y_train_boost = y_train_KNN;")
      st.write("y_test_boost = y_test_KNN;")
      st.write("功能：将之前任务（如 KNN）中已定义的训练集标签 y_train_KNN 和 测试集标签 y_test_KNN，赋值给新变量 y_train_boost 和 y_test_boost，作为 AdaBoost 模型的训练 / 测试标签；")
      st.write("目的：避免直接修改原始 KNN 数据，同时让变量名与当前 AdaBoost 任务对应，提高代码可读性。")
      st.write("（2）训练 AdaBoost 集成模型")
      st.write("x_train	输入的训练集特征数据（矩阵格式：行数 = 训练样本数，列数 = 特征数）；")
      st.write("y_train_boost	输入的训练集标签（向量格式：长度 = 训练样本数，标签需为离散类别值，如 1/2/3）；")
      st.write("'AdaBoostM2'	指定集成算法为 AdaBoostM2（AdaBoost 的多分类版本）；")
      st.write("注：AdaBoost 基础版（AdaBoostM1）仅支持二分类，而 AdaBoostM2 通过 “成对分类” 扩展到多分类，适合鸢尾花等多类别任务；")
      st.write("100	指定集成的弱学习器数量为 100（即迭代 100 次，每次训练 1 个弱学习器）；")
      st.write("数量越多通常模型性能越稳定，但计算成本会增加，需平衡；")
      st.write("'Tree'	指定弱学习器类型为 “决策树”（默认是深度为 1 的 “决策树桩”，即最简单的决策树，避免弱学习器过强导致集成失效）；")
      st.write("输出：boost 是训练完成的 AdaBoost 模型对象，包含所有弱学习器的参数、权重等信息。")
      st.write("（3）用 AdaBoost 模型预测测试集")
      st.write("y_boost = boost.predict(x_test);")
      st.write("功能：调用训练好的 AdaBoost 模型 boost，对测试集特征 x_test 进行分类预测；")
      st.write("输入：x_test 是测试集特征数据（格式与 x_train 一致：行数 = 测试样本数，列数 = 特征数）；")
      st.write("输出：y_boost 是测试集的预测标签向量（长度 = 测试样本数，每个元素对应一个样本的预测类别）。")
      st.write("（4）计算混淆矩阵评估性能")
      st.write("con_boost = confusionmat(y_test_boost, y_boost);")
      st.write("功能：通过 confusionmat 函数计算混淆矩阵，直观评估 AdaBoost 模型的分类效果；")
      st.write("输入：")
      st.write("y_test_boost：测试集的真实标签（ground truth）；")
      st.write("y_boost：测试集的预测标签；")
      st.write("输出：con_boost 是混淆矩阵（方阵，维度 = 类别数 × 类别数）：")
      st.write("矩阵的 行 代表 “真实类别”，列 代表 “预测类别”；")
      st.write("对角线元素：con_boost(i,i) 表示 “真实类别为 i 且预测正确的样本数”（正确分类数）；")
      st.write("非对角线元素：con_boost(i,j)（i≠j）表示 “真实类别为 i 但预测为 j 的样本数”（分类错误数）。")
      st.write("例如，若为 3 分类任务（如鸢尾花的 3 个品种），混淆矩阵可能如下：")
      st.write("plaintext")
      st.write("   10  0  0   % 真实类别1的10个样本全部预测正确")
      st.write("    0  9  1   % 真实类别2的9个样本正确，1个错分为类别3")
      st.write("    0  0 10   % 真实类别3的10个样本全部预测正确")
      st.write("3. 整体逻辑与核心思想")
      st.write("这段代码的完整流程是：复用已有数据 → 训练 AdaBoost 集成模型（100 个决策树弱学习器，多分类适配） → 预测测试集 → 用混淆矩阵评估性能")
      st.write("AdaBoost 的核心思想是 “关注错误样本”：")
      st.write("初始时，所有训练样本权重相同，训练第一个弱学习器；")
      st.write("每次迭代中，提高上一轮分类错误的样本的权重，降低分类正确样本的权重；")
      st.write("基于新的样本权重训练下一个弱学习器，并根据弱学习器的性能（准确率）分配权重；")
      st.write("迭代 100 次后，将所有弱学习器按权重 “投票”，最终结果作为集成模型的预测。")
    st.subheader("[查看代码运行结果]")
 ...
 # 4. 模型预测与评估
    ...
    st.subheader("🎲 推荐问题")
    if st.button("这段代码是什么意思呢"):
      st.write("1. 核心概念：Stacking")
      st.write("Stacking 是一种经典的集成学习策略，核心思路是 **“分两层学习”**：")
      st.write("第一层（基分类器层）：用多个不同类型的 “弱 / 强学习器”（如 KNN、SVM）对数据进行预测，输出各自的分类结果；")
      st.write("第二层（元分类器层）：将第一层所有基分类器的预测结果作为 “新特征”，训练一个 “元分类器”（如决策树），最终由元分类器输出最终预测。")
      st.write("这种方式能综合不同基分类器的优势，减少单一模型的偏差或方差，尤其适合数据复杂、单一模型性能有限的场景。")
      st.write("2. 逐行代码解释（结合 MATLAB 语法）")
      st.write("（1）前提说明")
      st.write("代码中 x_train/x_test（训练 / 测试集特征）、y_train/y_test（训练 / 测试集标签）是已预处理完成的输入数据（如数值型矩阵，无缺失值，标签为离散类别），需提前定义。")
      st.write("（2）第一层：训练两个基分类器")
      st.write("% 基分类器1：KNN（K近邻分类器）")
      st.write("mdl_knn = fitcknn(x_train, y_train, 'NumNeighbors', 5);")
      st.write("% 基分类器2：SVM（支持向量机，多分类版本）")
      st.write("mdl_svm = fitcecoc(x_train, y_train);")
      st.write("这一步是 Stacking 的 “基础层”，训练两个不同类型的基分类器，利用它们的互补性提升后续特征的信息量：")
      st.write("基分类器	函数与参数解析")
      st.write("KNN	- fitcknn：MATLAB 中训练 KNN 分类器的函数；")
      st.write("- 'NumNeighbors', 5：指定 “近邻数 K=5”（核心参数，控制 KNN 的复杂度）；")
      st.write("- 输出 mdl_knn：训练好的 KNN 模型对象。")
      st.write("SVM	- fitcecoc：MATLAB 中训练 “多分类 SVM” 的函数（全称：One-vs-One，通过 pairwise 方式处理多类别）；")
      st.write("- 默认使用 RBF 核（径向基函数），无需额外指定核函数时按默认配置；")
      st.write("- 输出 mdl_svm：训练好的多分类 SVM 模型对象。")
      st.write("（3）第一层：生成 “新特征”（基分类器的预测结果）")
      st.write("% 训练集的新特征：KNN和SVM对训练集的预测结果（各1列，合并为2列新特征）")
      st.write("x_train_new = [predict(mdl_knn, x_train), predict(mdl_svm, x_train)];")
      st.write("% 测试集的新特征：KNN和SVM对测试集的预测结果（格式与训练集新特征一致）")
      st.write("x_test_new = [predict(mdl_knn, x_test), predict(mdl_svm, x_test)];")
      st.write("这是 Stacking 的关键步骤 —— 将 “基分类器的预测结果” 转化为 “元分类器的输入特征”：")
      st.write("predict(mdl, data)：MATLAB 中模型预测函数，输入模型和数据，输出离散的分类标签（如 1/2/3，对应类别）；")
      st.write("x_train_new：维度为「训练样本数 × 2」（每一行是 1 个样本，两列分别是 KNN、SVM 对该样本的预测标签）；")
      st.write("x_test_new：维度为「测试样本数 × 2」（格式与 x_train_new 完全一致，确保元分类器输入特征匹配）。")
      st.write("（4）第二层：训练元分类器（决策树）")
      st.write("% 元分类器：简单决策树，输入为基分类器的预测结果（新特征），标签为原始训练标签")
      st.write("mdl_tree = fitctree(x_train_new, y_train);")
      st.write("fitctree：MATLAB 中训练 “分类决策树” 的函数（默认是简单的二叉决策树，无复杂剪枝，适合作为元分类器）；")
      st.write("输入 x_train_new（基分类器的预测结果）和 y_train（原始训练集标签）：元分类器的核心任务是 “学习如何组合基分类器的预测，以匹配真实标签”；")
      st.write("输出 mdl_tree：训练好的元分类器模型对象。")
      st.write("（5）Stacking 最终预测与性能评估")
      st.write("% 用元分类器预测测试集：输入测试集的新特征，输出最终分类结果")
      st.write("y_stack = predict(mdl_tree, x_test_new);")
      st.write("% 计算分类准确率：正确预测数 / 总测试样本数")
      st.write("accuracy = sum(y_stack == y_test) / numel(y_test);")
      st.write("disp(['Stacking Accuracy = ' num2str(accuracy)]);")
      st.write("% 计算混淆矩阵：直观展示预测正确/错误的类别分布")
      st.write("con_stack = confusionmat(y_test, y_stack);")
      st.write("这一步是 Stacking 流程的收尾，输出最终结果并评估性能：")
      st.write("最终预测：y_stack 是元分类器输出的 “Stacking 整体预测标签”（测试集每个样本的最终类别）；")
      st.write("准确率计算：")
      st.write("sum(y_stack == y_test)：统计 “预测标签与真实标签一致” 的样本数（正确数）；")
      st.write("numel(y_test)：统计测试集总样本数；")
      st.write("disp(...)：在 MATLAB 命令行打印准确率（如 Stacking Accuracy = 0.9667 表示 96.67% 准确率）；")
      st.write("混淆矩阵：")
      st.write("confusionmat(y_test, y_stack)：生成「类别数 × 类别数」的混淆矩阵；")
      st.write("对角线元素：某类别 “预测正确的样本数”；非对角线元素：某类别 “被误分为其他类别的样本数”，用于进一步分析分类错误的细节。")
      st.write("3. 整体流程总结")
      st.write("这段代码的完整逻辑可概括为：原始数据 → 训练KNN/SVM基分类器 → 基分类器输出预测结果（作为新特征） → 用新特征训练决策树元分类器 → 元分类器输出最终预测 → 评估准确率与混淆矩阵")
      st.write("4. 关键注意事项")
      st.write("基分类器多样性：代码选择 KNN（基于距离）和 SVM（基于核函数）作为基分类器，二者原理差异大，能提供互补信息 —— 这是 Stacking 有效的核心（若基分类器高度相似，集成增益会很低）；")
      st.write("数据一致性：训练集新特征 x_train_new 和测试集新特征 x_test_new 必须用 “同一批基分类器” 生成（不能重新训练基分类器），否则会导致数据泄露；")
      st.write("元分类器选择：代码用简单决策树作为元分类器（避免过拟合），实际应用中也可选择逻辑回归、随机森林等，但需注意元分类器复杂度（过复杂可能拟合基分类器的噪声）。")
